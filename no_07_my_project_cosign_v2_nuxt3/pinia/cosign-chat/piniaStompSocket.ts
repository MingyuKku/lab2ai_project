...
export const piniaStompSocket = defineStore(PiniaStompSocket, () => {
    ...
    // const updateMessage = (data?: ClassMessageHistory[], updateType?: 'init' | 'update') => {
    const updateMessage = (data?: ClassMessageHistory[]) => {
        if (!data) return;

        messages.value = data;
        // if (updateType === undefined || updateType === 'init') messages.value = data;
        // console.log('업데이트 메세지', updateType, data, messages.value)
        // if (updateType === 'update') messages.value = data.concat(messages.value);
    }
