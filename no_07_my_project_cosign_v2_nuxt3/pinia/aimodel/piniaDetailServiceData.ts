...
export const piniaDetailServiceData = defineStore(PiniaModelDetailServiceData, () => {
    ...
    const isLastList = ref<boolean | null>(null); // 마지막 싸인 리스트인지를 판별(sortType = created, updated, earningRate에서 모두 사용)
    
    const updateIsLastList = (value: boolean | null) => {
        isLastList.value = value;
    }
