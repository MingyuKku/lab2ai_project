<template>
    <div class="aimodel-page bg-bg-gray-04 pt-app-header">
        <CommonHeadersAppHeader>
            <template #left>
                <!-- <h1 v-if="isMobile" class="text-h1">AI 모델</h1> -->
                <IconBrandCosignFull />
            </template>
            <template #right>
        ...
        <div class="bg-bg-white pt-space-top-lg pb-space-page-bottom">
            <AimodelListModelItemFilter
                :refresh="refresh"
            />
            <div class="model-list">
                <AimodelListModelItem
                    v-for="(item,idx) in models"
                    :key="item.modelId"
                    :item="item"
                    :lazyLoad="idx > 2"
                    class="border-b-[1px] border-border-gray-02"
                />
            </div>
