<template>
    <div class="crypto-detail-page pt-app-header">
        <CommonHeadersAppHeader>
            <template #left>
                <div class="flex items-center">
                    <CommonHeadersBackHeaderIcon />
                    <div v-if="cryptoInfo" class="ml-2 flex items-center">
                        <div class="crypto-image w-6 h-6 mr-1">
                            <NuxtImg
                                :src="`${$config.public.IMAGE_URL}coin-image/v1/${cryptoInfo.cryptoCurrencyItem.digitalAssetsId}.png`"
                                :alt="cryptoInfo.cryptoCurrencyItem.digitalAssetsCode"
                                width="34" height="24"
                                format="webp"
                                loading="lazy"
                            />
                        </div>
                        <h1 class="text-h3 mt-px">{{ cryptoInfo.cryptoCurrencyItem.digitalAssetsKoName }}</h1>
                    </div>
                </div>
            </template>
        </CommonHeadersAppHeader>
        ...
        <div class="tab-contents-wrap bg-bg-white">
            ...
            >
                <CoinTabContentsPatterns
                    v-if="patternStatType && patternStat && buttonPatternId"
                    ...
                    :set-pattern-id="onClickButtonPatternId"
                    :interval="interval"
