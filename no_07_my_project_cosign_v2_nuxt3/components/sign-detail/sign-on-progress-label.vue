<template>
    <div class="progress-label relative">
        <div
            ...
            :class="labelClass"
        >{{ label }}</div>
        <div v-if="isPriceLabel" class="absolute flex items-end top-full translate-y-1 whitespace-nowrap" :class="target === 'target3' ? 'right-0' : 'x-center'">
            ...
            <div v-if="earning" class="flex items-center ml-1 leading-none text-price-profit">(<CommonPartsEarningRate :earning="earning" :size="14" class="leading-none" />)</div>
        </div>
    </div>
</template>
