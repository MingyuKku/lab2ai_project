<template>
    <div
        class="main-nav-bar fixed bottom-0 z-30 bg-font-black h-main-nav flex items-center justify-center"
...
import { BEST_PREDICT_MODEL_LIST, CHAT_GET_HISTORY, HOME_SIGN_ID_LIST, HOME_SIGN_ITEMS } from '~/composables/_app/asyncDataFetchKeys';
import { getCharacterNuxtDataKeys } from '~/composables/news/business/nuxtDataKeys';
...
const navItems = [
    ...
    {
        to: `/chat`,
        component: resolveComponent('IconNavMessageSend'),
        activeComponent: resolveComponent('IconNavActiveMessageSend'),
        label: '코싸인챗',
        name: 'chat',
        click: () => {
            clearNuxtData(CHAT_GET_HISTORY);
        }
    },
]
