...
import { messageWebview } from "../_app/useFlutterWebview";
...
export function useCoinTabIndex(paramCryptoId: number | null, setPatternVisible: (value: boolean) => void) {
    ...
    watch(
      ...
      (value) => {
        sendMixpanelCryptoPageTab(value as CryptoDetailPageQueryContentsType);
    ...
    const initTabSelection = async (queryTabIndex: CryptoDetailPageQueryContentsType | null, buttonPatternId: number | null) => {
        ...
        sendMixpanelCryptoPageTab(tabIndex.value);
        replace();
    }
    ...
    const sendMixpanelCryptoPageTab = (tabType: CryptoDetailPageQueryContentsType) => {
        const strCode = JSON.stringify({'CoinCode': cryptoId.value});

        if (tabType === 'timeline') messageWebview(`mixpanel/코인상세_타임라인/${strCode}`);
        if (tabType === 'pattern') messageWebview(`mixpanel/코인상세_차트패턴/${strCode}`);
        if (tabType === 'signs') messageWebview(`mixpanel/코인상세_SIGN/${strCode}`);
    }
