<template>
    <div class="sign-detail-page">
        ...
        <section v-if="isSignDetailContent && signProfile">
...
import { useAdType } from '~/composables/sign-detail/business/useAdType';
...
useAdType(signProfile.value?.adType, signProfile.value?.pay, signProfile.value?.tradeType);
mixpanelTradeType();

function mixpanelTradeType() {
    if (!signProfile.value?.tradeType) return;

    if (signProfile.value.tradeType === 'SPOT') messageWebview(`mixpanel/현물_SIGN상세/{"SubSequenceId": ${signid}, "AiModel": "${signProfile.value.modelId}"}`);
    if (signProfile.value.tradeType === 'FUTURES') messageWebview(`mixpanel/선물_SIGN상세/{"SubSequenceId": ${signid}, "AiModel": "${signProfile.value.modelId}"}`);
}
