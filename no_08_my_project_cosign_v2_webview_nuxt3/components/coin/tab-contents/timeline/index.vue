<template>
    <div 
        class="coin-detail-tab-contents-timeline "
        :class="{'pb-space-page-bottom': items.length > 0}"
    >
...
const clickTimelineItem = async (item: ICryptoTimelineItem) => {
    ...
    if(data?.patternScanTimelineItem) {
        ...
        // const realPath = (path.split('/').length === 2)? path.substring(0, path.lastIndexOf('/')): path;
        // push(`${realPath}/${data.patternScanTimelineItem.patternScanId}?content=analysis`);

        const { params, query } = useRoute();
        const { push } = useRouter();

        push({
            params: {...params, patternid: data.patternScanTimelineItem.patternScanId}, 
            query: {...query, tabIndex: "pattern"}
        })
        
    } else if(data?.signTimelineItem) {
        messageWebview(`https://cosign/sign_detail?modelId=${data?.signTimelineItem.modelId}&subSeqId=${data?.signTimelineItem.signSubSeqId}&isFree=${isFreeSign(false, true, 'END')}`)
