<template>
    <div>
        <div class="title">
            <label
                @click="!isGranted ? showGranted() : undefined"
                for="inputFile"
                class="d-button-st h-[50px] bg-bg-white border border-border-gray-04 text-font-dark-03" 
            >
...
const DOM_inputFile = ref<HTMLInputElement | null>(null);
const isGranted = ref<boolean>(false);

const setIsFileAccessGranted = (value: boolean) => {
    isGranted.value = value;
    if (value) {
        // 파일 업로드 창 띄우기
        DOM_inputFile.value?.click();
    }
}

const showGranted = () => {
    messageWebview(`check_file_access_granted`);
}

onMounted(() => {
    window.MessageHelper = {
        ...window.MessageHelper,
        setIsFileAccessGranted: setIsFileAccessGranted,
    }
})
