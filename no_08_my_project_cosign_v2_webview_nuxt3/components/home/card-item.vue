...
import type { UpbitSocketMessageItem } from '~/composables/home/business/_types';
...
const onClickSign = () => {
    const { item } = props;
    messageWebview(`https://cosign/sign_detail?modelId=${item.modelId}&subSeqId=${item.subSeqId}&isFree=${isFreeSign(item.ticket, item.pay, item.simpleStatusType)}`)
}
