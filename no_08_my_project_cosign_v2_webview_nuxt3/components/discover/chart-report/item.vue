...
const onClickCrypto = (item: RankMeta) => {
    // messageWebview(`2.0.0: {"major": "route","category": "coinDetail","title": "${item.digitalAssetsKoName}","params": {"digitalAssetId": ${item.digitalAssetsId},"cryptoCurrencyId": ${item.cryptoCurrencyId},"content": "timeline"}}`)
    messageWebview(`https://cosign/coin_detail/timeline?digitalAssetsId=${item.digitalAssetsId}&digitalAssetsName=${item.digitalAssetsKoName}&cryptoCurrencyId=${item.cryptoCurrencyId}`)
}
