...
export default {
  ...
  data() {
    return {
      informData: [
        {totalStat: 0, option: {countNum: 0, duration: 1}, fixPos: 0, icon: `${process.env.VUE_APP_IMG_URL}index/top/wonIcon_no_fill.svg`, title: '모든 예측에 투자했다면<span>수익률 합계</span>', type: '%'},
        {totalStat: 0, option: {countNum: 0, duration: 1}, fixPos: 1, icon: `${process.env.VUE_APP_IMG_URL}index/top/wonIcon_fill.svg`, title: '멘토들의 예측 정확도<span>수익 적중률</span>', type: '%'},
        {totalStat: 0, option: {countNum: 0, duration: 1}, fixPos: 0, icon: `${process.env.VUE_APP_IMG_URL}index/top/wonIcon_no_fill.svg`, title: '지금까지 COSIGN의<span>누적 SIGN 수</span>', type: '건'},
      ],
      loadedTimeFormatStr: '',
    }
  },
  ...
  methods: {
    async loadData() {
      const res = await axios.get(`${API_URL}/api/main/total_stats`);
