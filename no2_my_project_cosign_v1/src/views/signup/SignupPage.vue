  ...
  methods: {
    ...
    checkSignUp() {
      ...
      if(this.nickname.length === 0) {
        this.error.nickname.no_input = true;
        return false;
      } else if(!this.nickNameValid) {
        this.error.nickname.rule = true;
        return false;
      } else if(this.email.length === 0) {
        this.error.email.no_input = true;
        return false;
      } else if(this.validateEmail()) {
        this.error.email.rule = true;
        return false;
      } else if (this.password.length === 0) {
        this.error.password.no_input = true;
        return false;
      } else if (this.passwordCheck.length === 0) {
        this.error.password.check_no_input = true;
        return false;
      } else if(!this.passwordValid) {
        this.error.password.rule = true;
        return false;
      } else if(this.password !== this.passwordCheck) {
        this.error.password.not_same = true;
        return false;
      } else if(!this.agree.age || !this.agree.service || !this.agree.private) {
        this.error.terms_check = true;
        return false;
      } else {
        return true;
      }
    ...
    async signUp() {
      ...
      if(this.checkSignUp()) {
        ...
        const res = await axios.post(`${API_URL}/api/user/sign_up/join`, params.getParam(), {withCredentials: true});
