  ...
  <div class="talk_wrap" :class="{active: mobileActiveTabIdx === 1}">
    <div class="talk_body">
      ...
      <div class="talk_view_wrap" ref="talk_list">
        <TalkItem v-for='comment in comments' :key='comment.id' :comment='comment' :LoadComment="LoadComment" />
      </div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async LoadComment() {
        ...
        const res = await axios.post(`${API_URL}/api/mentor_page/comment/list/${this.mentorId}`, {}, {withCredentials: true});
      ...
      async LoadMoreComment() {
        if(this.comments.length >= 30) {
          ...
          const res = await axios.post(`${API_URL}/api/mentor_page/comment/list/page/${this.mentorId}/${lastId}`, {}, {withCredentials: true});
