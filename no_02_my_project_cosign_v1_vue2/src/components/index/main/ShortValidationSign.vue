<template>
  <div class="shortValidationSign_wrap">
    <div class="title">
      <!-- <div class="titIcon"><img :src="`${imgURL}index/main/shortTime_icon.svg`" alt="아이콘"></div> -->
      <h1>초 단타 SIGN</h1>
    </div>
    <div class="swiperSlide_wrap" v-if="this.items.length > 0">
      ...
      <div class="swiper-button-prev button BestExchangeSign-button-prev" slot="button-prev" v-if="allowNavFlag">
        <i class="xi-angle-left"></i>
      </div>
      <div class="swiper-button-next button BestExchangeSign-button-next" slot="button-next" v-if="allowNavFlag">
...
export default {
  ...
  data() {
    ...
    return {
      ...
      swiperOption: {
        ...
        touchRatio: 0.8,
        navigation: {
          nextEl: '.BestExchangeSign-button-next',
          prevEl: '.BestExchangeSign-button-prev'
        },
        on: {
          init() {
  ...
  methods: {
    async loadSign() {
      const res = await axios.get(`${API_URL}/api/main/shortHit_sign`);
...
<style lang='scss'>
  @import '@/components/index/main/css/shortValidationSign.scss';
</style>