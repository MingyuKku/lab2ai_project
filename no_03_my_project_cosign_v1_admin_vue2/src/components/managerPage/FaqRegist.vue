        ...
        <div class="category">
          ...
          <div class="select_wrap">
            <select v-model="selectCategory" @change="changeSelect">
              <option :value="data" :disabled="data === '선택'" v-for="data in category" :key="data">{{ data }}</option>
            </select>
...
import { api_postAddFaq_admin, api_postAddEditFile_faq } from '@/api/manager'
...
export default {
  ...
  mounted() {
    this.selectCategory = this.category[this.categoryIdx]
  },
  computed: {
  },
  methods: {
    ...
    async submitFaqData() {
      if(this.submitFlag) {
        ...
        await api_postAddFaq_admin(submit)
        .then(res=> {
          console.log('faq regist',res)
          this.$router.push(`/manager`).catch(() => {});
