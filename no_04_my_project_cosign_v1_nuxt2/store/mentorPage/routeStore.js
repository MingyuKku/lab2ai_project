    ...
    mutations: {
        ...
        setSignNow(state, {buttons, sign, activeSignSeqId}) {
            ...
            if(state.sign_now.buttons.find(button => button.subSeqId === activeSignSeqId) === undefined) {
                ...
                location.href = `/${state.mentor_id}/${buttons[0].subSeqId}`;
            } else if(activeSignSeqId !== undefined) {
                state.push_state_url = `/${state.mentor_id}/${activeSignSeqId}`;
            }
        },
    },
    actions: {
        async route({commit, dispatch}, {mentor_id, sign_seq_id}) {
            ...
            if(sign_seq_id !== undefined) {
                ...
                const res = await this.$axios.post(`/api/sign/get/route/${mentor_id}/${sign_seq_id}`, {}, {withCredentials: true});
                if(res.data.signNowRes !== null) {
                    ...
                    dispatch('mentorPage/signHistoryPopupStore/setSwipeItems', res.data.signHistoryRes, {root: true});
                    dispatch('mentorPage/signHistoryPopupStore/showHistoryPopup', {subSeqId: sign_seq_id}, {root: true});
                }
        ...
        async loadSignNow({commit}, {mentor_id}) {
            // 코인 이미지, 텍스트 가져와야 함
            const res = await this.$axios.post(`/api/sign/get/sign_now/current/${mentor_id}`, {}, {withCredentials: true});
        ...
        async loadAnotherSignNow({commit}, {sign_id}) {
            const res = await this.$axios.post(`/api/sign/get/sign_now/selected/${sign_id}`, {}, {withCredentials: true});
            const signBlurDto = res.data.signBlurDto;
            const signNowDto = res.data.signNowDto;
            const sign = signBlurDto !== null? signBlurDto: signNowDto;
