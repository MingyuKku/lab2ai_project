          ...
          <ul>
            ...
            <li class="earning">
              <span @click="sortingItems('targetEarningRateText')">목표수익률
                <i class="xi-caret-down" :class="{on: sortFlag['targetEarningRateText'], off: sortFlag['targetEarningRateText'] === false}"></i>
              </span>
            </li>
            <li class="validate">
              <span @click="sortingItems('validateMin')">유효기간
                <i class="xi-caret-down" :class="{on: sortFlag['validateMin'], off: sortFlag['validateMin'] === false}"></i>
              </span>
            </li>
            <li class="mentor">
              <span @click="sortingItems('mentorTitle')">멘토이름
                <i class="xi-caret-down" :class="{on: sortFlag['mentorTitle'], off: sortFlag['mentorTitle'] === false}"></i>
              </span>
            </li>
            <li class="time">
              <span @click="sortingItems('startTimeValue')">개시일
                <i class="xi-caret-down" :class="{on: sortFlag['startTimeValue'], off: sortFlag['startTimeValue'] === false}"></i>
              </span>
            </li>
          </ul>
        </div>
...
export default {
  ...
  methods: {
    // initSort() {
    //   this.items.sort((a,b) => b.startTimeValue - a.startTimeValue)
    // },
    initEarningWidth() {
      let maxLen = Number.MIN_SAFE_INTEGER;
      this.items.forEach((r)=> {
        maxLen = Math.max(maxLen, String(r.targetEarningRateText).split('.')[0].length)
        // if(String(r.targetEarningRateText).split('.')[0].length >= 3) {
        //   this.earningWidth = '92px';
        // } else if(String(r.targetEarningRateText).split('.')[0].length >= 2) {
