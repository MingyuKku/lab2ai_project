<template>
  <div class="shortValidationSign_wrap">
    ...
    <div class="swiperSlide_wrap" v-if="items.length > 0">
      ...
      <div class="swiper-button-prev button BestExchangeSign-button-prev" slot="button-prev" v-show="swiperNavFlag">
        <i class="xi-angle-left"></i></div>
      <div class="swiper-button-next button BestExchangeSign-button-next" slot="button-next" v-show="swiperNavFlag">
  ...
  async mounted() {    
    this.items = this.items.map(item => {
      return {
        ...
        endTimeText: Date.parseUtcText(item.startTime).addDate('minute', item.validateMin-1).format('HH:mm'),
      }
    })
  },
  methods: {
